Test stat frequence TEMRA chez  les patients avec un suivi longitudinal à VT1-4 et S1
...
# import datat------------------------------------------------------------------
rm(list = ls())
data <- read.xlsx("/Users/victor/Documents/JM/NanoString/Data_brut/cyto_V1_V4_S1_TEMRA_Agspe.xlsx", sheet = 3)
...
ggplot(TEMRA , aes(x = factor(Time, levels = c("VT1", "VT4", "S1")), 
                   y = CD8_TEMRA, 
                   fill =  Groupe)) +
  geom_boxplot() +
  ...
  # geom_jitter() +
  theme_classic() +
  labs(title = "Quantity of TEMRA patient with full longitudinal follow-up (V1-4 S1)",
       subtitle = "R = 8 / RP = 5",
