...
# 2 Import data-----------------------------------------------------------------
rm(list = ls())
PC2_VT1_T <- read.table("/Users/victor/Documents/JM/NanoString/IFN_covid_nano/data/PC2_VT1.txt")
PC2_VT2_T <- read.table("/Users/victor/Documents/JM/NanoString/IFN_covid_nano/data/PC2_VT2.txt")
...
data_fi <- inner_join(data_V1, PC2_VT1_T, by = c("numero_patient_nanostring" = "numero_patient",
                                                 "REPONSE"="REPONSE")) %>%
  filter(REPONSE != "T") %>%
...
for (namevar_i in namevars_to_plot) {
  print(namevar_i)
  p_i <- ggplot(
    data_fi, aes(x = SelectPCA.PC2, y = .data[[namevar_i]], color = REPONSE, label = label_patient)) +
    geom_point()+
...
mcor <- t(as.data.frame(cor(data_fi$SelectPCA.PC2, matcor, method= "pearson")))
...
list_plot <- list()
for (i in namecor) {
  pi<-cor.test(data_fi$SelectPCA.PC2, matcor[[i]], method= "pearson")
  list_plot[[i]] <- pi
}
for (p in list_plot) {
  print(p)
...
data_fi <- inner_join(data_V1, PC2_VT2_T, by = c("numero_patient_nanostring" = "numero_patient",
                                                 "REPONSE"="REPONSE")) %>%
  filter(REPONSE != "T") %>%
  rename(numero_patient = "numero_patient_nanostring") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
matcor <- cbind(data_fi$`Age à S1`, data_fi[11:50])
colnames(matcor)[1] <- "age"
mcor <- t(as.data.frame(cor(data_fi$SelectPCA.PC2, matcor)))
...
data_fi <- inner_join(data_6m, PC2_VT1_T, by = c("numero_patient_nanostring" = "numero_patient",
                                                 "REPONSE"="REPONSE")) %>%
  filter(REPONSE != "T") %>%
  rename(numero_patient = "numero_patient_nanostring") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
for (namevar_i in namevars_to_plot) {
  ...
  data_clean$SelectPCA.PC2 <- data_fi$SelectPCA.PC2
  data_clean <- na.omit(data_clean)
  # plot 
  p_i <- ggplot(
    data_clean, aes(x = SelectPCA.PC2, y = .data[[namevar_i]], color = REPONSE, label = label_patient)) +
    geom_point()+
    ggrepel::geom_text_repel(nudge_x=0.6,
                             # nudge_y=0.15,
                             show.legend = F,
...
for (namevar_i in namevars_to_plot) {
  ...
  p_i <- t(as.data.frame(cor(data_clean$SelectPCA.PC2, data_clean[1])))
...
for (namevar_i in namevars_to_plot) {
  ...
  p_i <- cor.test(data_clean$SelectPCA.PC2, data_clean[[namevar_i]], method= "pearson")
...
data_fi <- inner_join(data_6m, PC2_VT2_T, by = c("numero_patient_nanostring" = "numero_patient",
                                                 "REPONSE"="REPONSE")) %>%
  filter(REPONSE != "T") %>%
  rename(numero_patient = "numero_patient_nanostring") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
data_fi <- inner_join(data_V4_CD4_8_Agspe, PC2_VT1_T, by = c("numero_patient" = "numero_patient",
                                                 "Responder"="REPONSE")) %>%
  # filter(REPONSE != "T") %>%
...
data_fi <- inner_join(data_V4_CD4_8_Agspe, PC2_VT2_T, by = c("numero_patient" = "numero_patient",
                                                            "Responder"="REPONSE")) %>%
  # filter(REPONSE != "T") %>%
  rename(REPONSE = "Responder") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
data_fi <- inner_join(data_V4_T_CD4_8_stim, PC2_VT1_T, by = c("numero_patient" = "numero_patient",
                                                             "Responder"="REPONSE")) %>%
  # filter(REPONSE != "T") %>%
  rename(REPONSE = "Responder") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
data_fi <- inner_join(data_V4_T_CD4_8_stim, PC2_VT2_T, by = c("numero_patient" = "numero_patient",
                                                             "Responder"="REPONSE")) %>%
  # filter(REPONSE != "T") %>%
  rename(REPONSE = "Responder") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
data_fi <- inner_join(data_V4_T_CD4_8_no_stim, PC2_VT1_T, by = c("numero_patient" = "numero_patient",
                                                              "Responder"="REPONSE")) %>%
  # filter(REPONSE != "T") %>%
  rename(REPONSE = "Responder") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
...
data_fi <- inner_join(data_V4_T_CD4_8_no_stim, PC2_VT2_T, by = c("numero_patient" = "numero_patient",
                                                              "Responder"="REPONSE")) %>%
  # filter(REPONSE != "T") %>%
  rename(REPONSE = "Responder") %>%
  mutate(label_patient = as.character(numero_patient)) %>%
