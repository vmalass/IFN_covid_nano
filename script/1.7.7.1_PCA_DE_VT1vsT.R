...
## 4.2-Visualisation------------------------------------------------------------
fviz_eig(PCA, main = "Variance par PC à VT1 sur le gene DE VT1vsT", addlabels = TRUE)
fviz_pca_var(PCA,
...
ggplot(mergeMetaAPC, aes(x = PC1, y = PC2, color = REPONSE, label = label)) +
  geom_point(size = 3) + 
  ...
  labs(title="PCA à partir des prelevement VT1 et des genes DE VT1vsT") + 
...
write.table(PC1_VT1, "data/PC1_VT1_gene_DE_VT1vsT_mat1.3.txt")

ggplot(PC1_VT1, aes(x =jours_prelevement , y = `SelectPCA$PC1`, color = REPONSE, label = label)) +
  geom_point(size = 3) + 
  geom_text_repel(show.legend = F,
                  max.overlaps  = Inf, 
                  size = 5)	+
  ...
  scale_color_manual(breaks = c("NR", "NR-", "R", "RP-", "RP", "T", "A"),
                     values = c("#7570BE", "darkorange","cornflowerblue",
                                "#882255" ,"brown3", "chartreuse4", "#BBBBBB")) +
  labs(title="PCA à partir des prelevement VT1 et des genes DE VT1vsT en fonction des jours de prélèvement",
...
# 5-PCA VT1 gene DE VT1vsT en VT2-----------------------------------------------
## 5.1-Dataframe----------------------------------------------------------------
mat_VT1 <- mat_DE_VT1[which(mat_DE_VT1$real_time_point %in% "VT2"),]
mat <- rbind(mat_DE_VT1[which(mat_DE_VT1$real_time_point %in% "T"),], mat_VT1)
mat<- arrange(mat,numero_patient) #ordo par n° pat
...
## 5.2-Visualisation------------------------------------------------------------
fviz_eig(PCA, main = "Variance par PC à VT2 sur le gene DE VT1vsT", addlabels = TRUE)
fviz_pca_var(PCA,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, # Avoid text overlapping
...
ggplot(mergeMetaAPC, aes(x = PC1, y = PC2, color = REPONSE, label = label)) +
  ...
  scale_color_manual(breaks = c("NR", "NR-", "R", "RP-", "RP", "T", "A"),
                     values = c("#7570BE", "darkorange","cornflowerblue",
                                "#882255" ,"brown3", "chartreuse4", "#BBBBBB")) +
  labs(title="PCA à partir des prelevement VT2 et des genes DE VT1vsT") + 
  theme(plot.title = element_text(size=24),
        axis.title = element_text(size = 24),
        axis.text = element_text(size = 18),
        legend.title = element_text(size=15),
...
## 5.3-Extraction PC1 VT1-------------------------------------------------------
PC1_VT1 <- as.data.frame(SelectPCA$PC1)
row.names(PC1_VT1) <- rownames(SelectPCA)
all(rownames(PC1_VT1) == rownames(MataData_PCA))
PC1_VT1 <- cbind(PC1_VT1, MataData_PCA)
write.table(PC1_VT1, "data/PC1_VT2_gene_DE_VT1vsT_mat1.3.txt")
...
ggplot(PC1_VT1, aes(x =jours_prelevement , y = `SelectPCA$PC1`, color = REPONSE, label = label)) +
  ...
  scale_color_manual(breaks = c("NR", "NR-", "R", "RP-", "RP", "T", "A"),
                     values = c("#7570BE", "darkorange","cornflowerblue",
                                "#882255" ,"brown3", "chartreuse4", "#BBBBBB")) +
  labs(title="PCA à partir des prelevement VT2 et des genes DE VT1vsT en fonction des jours de prélèvement",
       x = "jours après début des symptômes",
       y = "PC1") + 
  theme(plot.title = element_text(size=24),
        axis.title = element_text(size = 24),
...
## 7.2-Visualisation------------------------------------------------------------
fviz_eig(PCA, main = "Variance par PC avec les genes DE VT1vsT au time point V1", addlabels = TRUE)
fviz_pca_var(PCA,
             col.var = "contrib", # Color by contributions to the PC
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE, # Avoid text overlapping
...
ggplot(mergeMetaAPC, aes(x = PC1, y = PC2, color = REPONSE, shape = real_time_point, label = label)) +
  ...
  scale_color_manual(breaks = c("NR", "NR-", "R", "RP-", "RP", "T", "A"),
                     values = c("#7570BE", "darkorange","cornflowerblue",
                                "#882255" ,"brown3", "chartreuse4", "#BBBBBB")) +
  labs(title="PCA avec les genes DE VT1vsT au time point V1",
       x = "PC1 = 74.7%",
       y = "PC2 = 6.4%") + 
  theme(plot.title = element_text(size=24),
        axis.title = element_text(size = 24),
...
## 7.3-Extraction PC1 & visualisation-------------------------------------------
PC1_VT1 <- as.data.frame(SelectPCA$PC1)
row.names(PC1_VT1) <- rownames(SelectPCA)
all(rownames(PC1_VT1) == rownames(MataData_PCA))
PC1_VT1 <- cbind(PC1_VT1, MataData_PCA)
write.table(PC1_VT1, "data/PC1_V1_gene_DE_VT1vsT_mat1.3.txt")
